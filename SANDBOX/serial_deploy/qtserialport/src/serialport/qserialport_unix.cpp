...
bool QSerialPortPrivate::open(QIODevice::OpenMode mode)
{
    ...
    vtrace << "qt_safe_open(systemLocation.toLocal8Bit().constData(), flags);";
    ...
    if (!initialize(mode)) {
        vtrace << "qt_safe_close(descriptor);";
...
//    Q_Q(QSerialPort);
    ...
    vtrace << "char *ptr = buffer.reserve(bytesToRead);";
...
bool QSerialPortPrivate::startAsyncWrite()
{
    ...
    vtrace << "qint64 written = writeToPort(writeBuffer.readPointer(), writeBuffer.nextDataBlockSize());";
...
{
    vtrace << "qt_safe_read(descriptor, data, maxSize);";
